...
- **アクセス制御**: 
  - **バックエンド**: 管理者または申請者（作成者本人）のみアクセス可能
  - **フロントエンド**: 
    - 管理者: 全フォームのチャット機能にアクセス可能
    - 一般ユーザー: 自分が作成した申請中・承認済み・却下状態のフォームのチャット機能にアクセス可能（公開中のフォームは一覧に表示されない）
- **チャット表示条件（フロントエンド）**:

  | フォーム状態 | 管理者 | 申請者（作成者） | 詳細 |
  | :--- | :---: | :---: | :--- |
  | `pending`（申請中） | ✅ | ✅ | 申請内容の補足説明や管理者への質問が可能 |
  | `rejected`（却下） | ✅ | ✅ | 却下理由の確認や修正相談が可能 |
  | `approved` + `is_active=False`（承認済み・停止中） | ✅ | ❌ | チャット欄は非表示（編集・削除は不可） |
  | `approved` + `is_active=True`（公開中） | ✅ | ❌ | 申請者の一覧に表示されない（チャット不要） |
