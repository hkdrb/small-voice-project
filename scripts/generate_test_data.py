...
VALUES_STATEMENT_PREFIXES = [
    "私は", "仕事では", "常に", "何よりも", "特に", 
    "日々の業務で", "チームで働く上で", "プロとして", ""
]

VALUES_STATEMENT_CORES = [
    "誠実であることを大切にしています",
    ...
    "感謝の気持ちとリスペクトを忘れないようにしています",
    "仕事を楽しむ心を持ち続けたいと思っています",
    "論理的に考えることを大切にしています",
    "継続的に学び続ける姿勢を重視しています",
    "心理的安全性を大事にしています",
    ...
    "当事者意識を持つことを大切にしています",
    "透明性を重視しています",
    "多様性を受け入れることを大事にしています",
    "効率を追求することを心がけています",
    "品質にこだわることを大切にしています"
]

VALUES_STATEMENT_SUFFIXES = [
    "。", "。", "。", "。", "。",
    "と考えています。", "と思っています。", "と信じています。"
]
...
def get_value_comment_q1():
    """Q1: あなたが仕事をする上で、大切にしている価値観を教えてください"""
    prefix = random.choice(VALUES_STATEMENT_PREFIXES)
    core = random.choice(VALUES_STATEMENT_CORES)
    suffix = random.choice(VALUES_STATEMENT_SUFFIXES)
    
    # Combine parts naturally
    if prefix:
        return f"{prefix}{core}{suffix}"
    else:
        # Capitalize first letter if no prefix
        return f"{core[0].upper()}{core[1:]}{suffix}"
