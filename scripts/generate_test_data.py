...
# --- Q1 Combinatorial Parts ---
Q1_INTROS = [
    "私が仕事をする上で、", "常に心がけているのは、", "プロフェッショナルとして、", 
    "チームの一員として、", "個人的な信念として、", "これまでの経験から、", 
    "何よりも優先しているのは、", "私の行動指針は、", "譲れない価値観として、",
    "日々の業務において、", "社会人として、", "自分自身のルールとして、"
]

Q1_CORE = {
    "感謝・称賛": [
        "感謝の気持ちを言葉にすること", "仲間の貢献を称賛すること", "リスペクトを持ち続けること", 
        "チームワークと相互尊重", "「ありがとう」を伝えること", "互いの強みを認め合うこと",
        "ポジティブなフィードバック", "心理的な支え合い"
    ],
    "挑戦": [
        "新しいことへの挑戦", "失敗を恐れない姿勢", "現状維持を疑うこと", 
        "高い目標へのチャレンジ", "未知の領域への探求", "イノベーションの追求",
        "変化を楽しむこと", "リスクを取って進むこと"
    ],
    "心理的安全性": [
        "心理的安全性の確保", "透明性の高いコミュニケーション", "率直な意見交換", 
        "情報を隠さないこと", "弱みを見せられる環境づくり", "風通しの良さ",
        "バッドニュース・ファースト", "誠実な対話"
    ],
    "顧客・品質": [
        "顧客第一主義", "圧倒的な品質へのこだわり", "ユーザー視点の徹底", 
        "プロとしての責任感", "期待を超える価値提供", "細部へのこだわり",
        "誠実な顧客対応", "妥協しないモノづくり"
    ],
    "スピード": [
        "圧倒的なスピード", "迅速な意思決定", "効率の最大化", 
        "無駄の排除", "自律的な行動", "即断即決",
        "スピードと質のバランス", "Time to Marketの意識"
    ]
}

Q1_PREDICATES = [
    "を大切にしています。", "を最も重視しています。", "を心がけています。", 
    "を軸に働いています。", "を信念としています。", "に重きを置いています。", 
    "を実践するようにしています。", "を忘れないようにしています。",
    "が一番大事だと考えています。"
]

Q1_REASONS = {
    "感謝・称賛": [
        "それがチームの信頼関係を作るからです。", "感謝がモチベーションになるからです。", 
        "一人では大きな仕事はできないからです。", "良い雰囲気こそが成果を生むからです。",
        "信頼関係が全ての土台だからです。"
    ],
    "挑戦": [
        "成長は挑戦の先にしかないからです。", "失敗こそが学びの源泉だからです。",
        "変化しないことは退化と同じだからです。", "常に進化し続けたいからです。",
        "新しい価値を創造したいからです。"
    ],
    "心理的安全性": [
        "隠し事はいつか大きな問題になるからです。", "安心して発言できる場が必要だからです。",
        "信頼がないとチームは機能しないからです。", "早期の問題発見に繋がるからです。",
        "健全な議論が良いプロダクトを作るからです。"
    ],
    "顧客・品質": [
        "顧客の信頼こそが資産だからです。", "品質はプロのプライドだからです。",
        "使い手に喜んでもらうことが喜びだからです。", "長期的な信頼に繋がるからです。",
        "それがビジネスの根本だからです。"
    ],
    "スピード": [
        "時間は有限なリソースだからです。", "市場は待ってくれないからです。",
        "早く失敗して早く学ぶのが効率的だからです。", "機会損失を防ぎたいからです。",
        "成果を最速で届けたいからです。"
    ]
}

def generate_q1_sentences(theme_key, count=100):
    sentences = set()
    attempts = 0
    # Map 'スピード・効率' to 'スピード'
    key = "スピード" if theme_key == "スピード・効率" else theme_key
    
    while len(sentences) < count and attempts < count * 5:
        intro = random.choice(Q1_INTROS) if random.random() > 0.3 else ""
        core = random.choice(Q1_CORE[key])
        pred = random.choice(Q1_PREDICATES)
        reason = random.choice(Q1_REASONS[key]) if random.random() > 0.5 else ""
        
        sentence = f"{intro}{core}{pred} {reason}".strip()
        sentences.add(sentence)
        attempts += 1
    return list(sentences)

VALUES_THEMES_GROUPED = {
    "感謝・称賛": generate_q1_sentences("感謝・称賛", 150),
    "挑戦": generate_q1_sentences("挑戦", 150),
    "心理的安全性": generate_q1_sentences("心理的安全性", 150),
    "顧客・品質": generate_q1_sentences("顧客・品質", 150),
    "スピード・効率": generate_q1_sentences("スピード・効率", 150)
}
...
DENSE_CLUSTERS = {
    ...
    "values_theme": [
        {"topic": "感謝・称賛", "templates": VALUES_THEMES_GROUPED["感謝・称賛"]},
        {"topic": "挑戦", "templates": VALUES_THEMES_GROUPED["挑戦"]},
        {"topic": "心理的安全性", "templates": VALUES_THEMES_GROUPED["心理的安全性"]},
        {"topic": "顧客・品質", "templates": VALUES_THEMES_GROUPED["顧客・品質"]},
        {"topic": "スピード", "templates": VALUES_THEMES_GROUPED["スピード・効率"]}
    ]
}
