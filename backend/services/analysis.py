...
def get_batch_sentiments(texts):
    ...
    for i in range(0, len(texts), batch_size):
        ...
        prompt = f"""
あなたは世論調査の分析官です。
以下の意見リストを読み、その内容が**一般的に見て「ポジティブ（肯定的）」か「ネガティブ（批判的）」か**を判定し、スコア（-1.0 〜 1.0）を付けてください。

### 判定基準:
1. **内容重視**: 言葉遣いの丁寧さや表現の婉曲さではなく、**「意見の実体」**を見てください。
   - どんなに丁寧な言葉でも、現状への不満や問題点の指摘が含まれていれば **「ネガティブ」** です。
   - 多少ラフな言葉でも、感謝や同意、満足を示していれば **「ポジティブ」** です。
2. **分類の指針**:
   - **ネガティブ (-1.0 〜 -0.1)**:
     - 「〜してほしい」「〜が足りない」（改善要望＝現状の不足）
     - 「困っている」「納得できない」（不満、拒絶）
     - 「不安だ」「リスクがある」（懸念）
   - **ポジティブ (0.1 〜 1.0)**:
     - 「ありがとう」「助かった」（感謝）
