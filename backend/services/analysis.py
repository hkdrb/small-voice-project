...
以下の社員の声（日本語）のリストを読み、各発言の感情スコア（-1.0 〜 1.0）を決定してください。

### 【最重要ルール】:
1. **中立(0.0)の禁止**: 組織内の発言において、完全な中立は稀です。**「0.0」は、日時や場所などの「純粋な事実の報告」以外には使用しないでください。**
2. **微細な感情の強制検出**: 
   - 「特にありません」「現状維持で良い」などは、組織への関心の低さや諦めを内包しているため、**必ずネガティブ（-0.1 〜 -0.3）**として判定してください。
   - 改善要望（〜してほしい）は、裏を返せば現状への不満であるため、**必ずネガティブ（-0.2 〜 -0.5）**として判定してください。
   - わずかな感謝や肯定でも、ポジティブ側に倒してください。
3. **二極化の促進**: 迷った場合は、中立に置かず、ポジティブかネガティブかのいずれかの側に微量でも倒してください（例: -0.15, 0.12 など）。

### スコア指標:
- **-1.0 〜 -0.6**: 強い憤り、メンタル不調、離職の意志、不正への警鐘、深刻な非効率への怒り。
- **-0.5 〜 -0.1**: 違和感、改善要望、現状への諦め、不便さの指摘、組織への冷淡さ。
- **0.1 〜 0.5**: 感謝、前向きな提案、改善への意欲、微かな手応え。
- **0.6 〜 1.0**: 高いモチベーション、組織への強い信頼、卓越した成果への喜び。

### 出力形式:
JSON配列のみを出力してください。
例: [-0.45, -0.12, 0.3, -0.05, 0.72] (0.0は極力避けること)
