...
4. **言語**: 分析結果や議題の作成など、すべての出力は必ず日本語で行ってください。
...
議論を停滞させず、具体的かつ実行可能な解決策や次にとるべきステップを最大3つ提示してください。
   ...
   - **detail**: そのアクションの具体的な内容。なぜそのアクションが必要なのかという背景（論点）も簡潔に含めてください。
   - **言語**: 必ず日本語で出力してください。
...
### 制約事項:
- 提案されるフォームの質問はすべて「自由記述」形式となります。
- そのため、質問文の末尾に「（5段階評価）」や「（選択式）」といった回答形式の指定を含めないでください。单纯な質問文のみを記述してください。
